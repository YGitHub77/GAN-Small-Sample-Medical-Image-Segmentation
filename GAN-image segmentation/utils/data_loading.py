...
class BasicDatasetFromPaths(Dataset):
    def __init__(self, image_paths, mask_paths, scale=1.0):
        ...
        for path in self.mask_paths:
            img = load_image(path)
            unique.append(np.unique(np.asarray(img)))

        self.mask_values = list(sorted(np.unique(np.concatenate(unique), axis=0).tolist()))
        logging.info(f"[BasicDatasetFromPaths] Unique mask values: {self.mask_values}")
    ...
    def __getitem__(self, idx):
        ...
        img = load_image(img_file).convert('RGB')
        mask = load_image(mask_file).convert('L')
